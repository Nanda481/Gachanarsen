 ...
 * - Added: Loading screen dengan progress bar
 * - Added: Vertical dan horizontal scrolling
 * - Added: Footer dengan credit
    ...
    finishLoading() {
        ...
        setTimeout(() => {
            ...
            if (this.scrollableContent) {
                this.scrollableContent.style.overflowY = 'auto';
            }
...
class ScrollSystem {
    constructor() {
        this.horizontalContainer = document.getElementById('horizontalScroll');
        this.scrollRange = document.createElement('input');
        this.init();
    }
    ...
    createScrollControls() {
        // Create range slider untuk horizontal scroll
        this.scrollRange.type = 'range';
        this.scrollRange.className = 'scroll-range';
        this.scrollRange.min = '0';
        this.scrollRange.max = '100';
        this.scrollRange.value = '0';

        // Create controls container
        const controlsContainer = document.createElement('div');
        controlsContainer.className = 'horizontal-scroll-controls';
        controlsContainer.appendChild(this.scrollRange);

        document.querySelector('.center').appendChild(controlsContainer);
    }

    setupEventListeners() {
        // Horizontal scroll dengan range slider
        this.scrollRange.addEventListener('input', (e) => {
            const scrollPercentage = parseInt(e.target.value);
        ...
        // Mouse wheel untuk horizontal scroll (dengan modifier key)
        this.horizontalContainer.addEventListener('wheel', (e) => {
            if (e.shiftKey) {
                e.preventDefault();
                this.horizontalContainer.scrollLeft += e.deltaY;
                this.updateRangeSlider();
            }
        });

        // Update range slider saat scroll
        this.horizontalContainer.addEventListener('scroll', () => {
            this.updateRangeSlider();
        });
    }

    scrollToPercentage(percentage) {
        const maxScroll = this.horizontalContainer.scrollWidth - this.horizontalContainer.clientWidth;
        const targetScroll = (percentage / 100) * maxScroll;
        this.horizontalContainer.scrollTo({
            left: targetScroll,
            behavior: 'smooth'
        });
    }
    ...
    updateRangeSlider() {
        const maxScroll = this.horizontalContainer.scrollWidth - this.horizontalContainer.clientWidth;
        if (maxScroll > 0) {
            const scrollPercentage = (this.horizontalContainer.scrollLeft / maxScroll) * 100;
            this.scrollRange.value = scrollPercentage;
        }
    }
}
...
function cleanupAllSystems() {
  ...
  if (window.loadingSystem) {
    // Cleanup loading system jika diperlukan
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  try {
    // Initialize loading system pertama
    window.loadingSystem = new LoadingSystem();
    window.loadingSystem.init();

    // Initialize systems lainnya setelah loading
    setTimeout(() => {
